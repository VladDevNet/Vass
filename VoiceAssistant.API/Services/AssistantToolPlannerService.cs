...
public sealed class AssistantToolPlannerService
{
    ...
    internal static string BuildPlannerInstructions(string systemPrompt) => $$"""
                            ...
                            Когда пользователь явно просит отключиться, не слушать, оставить
                            его одного или прощается, завершая разговор (например, «пока»,
                            «я поговорю с кем-то», «я пошла заниматься своими делами»), вызови
                            assistant_sleep. После его успешного результата коротко подтверди,
                            что ставишь слушание на паузу. Не используй этот инструмент для
                            простого упоминания таких слов или для вопроса о том, как работает пауза.
    ...
    [
        ...
        new { name = "assistant_sleep", description = "Отключает активное голосовое слушание по явной просьбе пользователя. Используй, когда он завершает разговор: «пока», «отключайся», «я поговорю с кем-то», «я займусь своими делами». После этого слушание возобновляется только при открытии Vass или нажатии микрофона.", parameters = EmptyObjectSchema() },
