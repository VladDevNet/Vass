...
Все анимации — с `useNativeDriver: true` (как уже в `AvatarFace.tsx`): мимика идёт на отдельном потоке, не на JS-треде, где крутится голосовой цикл — рассинхронизация тут была бы не просто визуальным багом, а потенциальной причиной пропущенных VAD-тиков.
