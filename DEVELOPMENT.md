...
Core user flow: the user speaks, the mobile app records/detects voice activity, the backend transcribes and runs the turn through Gemini (with Google Search grounding), and the reply is spoken back on-device via the system TTS engine (`expo-speech`). There is deliberately no server-side TTS fallback: a local synthesis failure leaves the already-visible reply as text and returns the voice loop to a clean listening state instead of introducing a long network delay. Real turn-taking (deciding whether a pause means the user is done vs. still thinking) and interruption handling live entirely in the mobile client — see `docs/react-native/audio-and-vad.md` and `docs/react-native/tts-and-avatar.md` for the current algorithm, which has evolved through several PRs since the initial design.

Secondary features:

- Text chat with SSE streaming, independent of voice mode.
- Camera/image OCR through Gemini.
- Per-user long-term memory: Gemini embeddings + pgvector semantic retrieval, model-chosen structured categories, and optional retained photo/document attachments that the user can view later.
- Local-first reminders: server-side language/date parsing plus OS-scheduled mobile notifications that fire offline.
- Visual input: camera/gallery/file picker, Android screen capture with OS consent, and system share of text, links, images, PDFs, and other documents into Vass.
- Capability help: the model and mobile settings expose the same feature catalog, natural-language examples, and interface hints.
...
| Speech synthesis | On-device system TTS through Expo Speech (`expo-speech`) |
...
`speaker-id` (SpeechBrain-based voice identification) is in `docker-compose.yml` but opt-in only (`profiles: ["speaker-id"]` -- `docker compose up` alone never starts it, and `api` does not `depends_on` it). The abandoned Silero comparison is not part of the runtime. See below.
...
│   │   ├── ChatController.cs        # api/v1/chat — one companion session, SSE chat, audio, visual input
│   │   ├── CapabilitiesController.cs # api/v1/capabilities — help catalog for the mobile UI
│   │   ├── ClientLogsController.cs  # api/v1/client-logs — mobile client log ingest (dev tooling)
│   │   ├── MemoryController.cs      # api/v1/memory — structured personal memory and retained attachments
...
│   ├── Data/
│   │   ├── AppDbContext.cs
│   │   └── Entities/                # ChatSession, Message, UserSettings, MemoryItem/VisualAsset, Reminder/Delivery
│   ├── Migrations/
│   ├── Prompts/
...
- Application services: Gemini, companion prompt, memory/tool broker, audio analysis, speaker-ID (call site is real, uncommented code — `SpeakerRegistryService.IdentifyAsync` no-ops immediately unless `Features:SpeakerIdentificationEnabled` is set, see below).
...
| --- | --- | --- |
| `AuthController` | `/api/v1/auth` | Register, login, device-link, current user; supports pending approval when registration is closed |
| `AdminController` | `/api/v1/admin` | Admin-only user activity, access approval, search, sorting, and pagination |
| `ChatController` | `/api/v1/chat` | Single companion session, SSE chat, audio upload/playback, visual assets, utterance-completeness check, OCR |
| `CapabilitiesController` | `/api/v1/capabilities` | Feature catalog with examples and interface hints for the mobile help screen |
...
| Method | Path | Notes |
| --- | --- | --- |
| `POST` | `/api/v1/auth/register` | Creates user and returns JWT, or `202` with `approvalRequired=true` when closed registration needs approval |
| `POST` | `/api/v1/auth/login` | Returns JWT; an unapproved account receives `403` / `approval_required` |
...
| `POST` | `/api/v1/chat/visual-assets` | Stores an authenticated user-owned image or document before a chat turn |
| `GET` | `/api/v1/chat/visual-assets/{id}/content` | Streams a user-owned retained attachment for image/document viewing |
...
| Method | Path | Notes |
| --- | --- | --- |
| `GET` | `/api/v1/memory/items` | Lists active user-owned structured memory, including an optional retained attachment receipt |
| `GET` | `/api/v1/memory/search?query=...` | Semantic/lexical search over the user's own memory |
| `POST` | `/api/v1/memory/remember` | Saves one verified memory item with an optional category |
| `POST` | `/api/v1/memory/correct` | Creates a corrected successor to one remembered item |
| `POST` | `/api/v1/memory/forget` | Tombstones one active memory item |
| `POST` | `/api/v1/memory/clear/prepare`, `/clear` | Clears all active memory only after a bound confirmation token |
...
| `GET` | `/api/v1/reminders/manage` | All active reminders for the settings management screen and model tools |
...
| Method | Path | Notes |
| --- | --- | --- |
| `GET` | `/api/v1/capabilities/help` | Returns capability cards filtered by the current client's declared support; the model receives the same catalog through `capability_help` |
...
POST /api/v1/chat/upload-audio, then POST /api/v1/chat/send with audioFileName
  ...
  -> spoken through on-device system TTS (mobile)
```
...
- `REGISTRATION_AUTO_APPROVE` defaults to `true`. When it is `false`, registration returns a pending state and the mobile client shows a dedicated approval-waiting screen.
...
### Full Docker Stack (backend + admin + db)
...
- On-device system TTS only: no server voice runtime or network fallback is kept in production.
